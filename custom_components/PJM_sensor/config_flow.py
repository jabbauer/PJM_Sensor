...
SENSOR_OPTIONS = {
    ...
    "zonal_lmp": "Zonal LMP",
    "coincident_peak_prediction": "Coincident Peak Prediction",
}
...
class PJMConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def _build_schema(self, defaults=None):
        defaults = defaults or {"zone": "PJM-RTO", "peak_threshold": DEFAULT_PEAK_THRESHOLD, "accuracy_threshold": DEFAULT_ACCURACY_THRESHOLD}
        schema_dict = {
        ...
        for sensor_id, sensor_label in SENSOR_OPTIONS.items():
            # Preselect system sensors and the new peak prediction sensor by default
            default_value = defaults.get(sensor_id, sensor_id in SYSTEM_SENSORS or sensor_id == "coincident_peak_prediction")
...
class PJMOptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, user_input=None):
        ...
        current_peak_threshold = self.config_entry.data.get("peak_threshold", DEFAULT_PEAK_THRESHOLD)
        current_accuracy_threshold = self.config_entry.data.get("accuracy_threshold", DEFAULT_ACCURACY_THRESHOLD)
    ...
    def _build_schema(self, data):
        ...
        for sensor_id, sensor_label in SENSOR_OPTIONS.items():
            schema_dict[vol.Optional(sensor_id, default=data.get(sensor_id, sensor_id in SYSTEM_SENSORS or sensor_id == "coincident_peak_prediction"))] = bool
        return vol.Schema(schema_dict)
